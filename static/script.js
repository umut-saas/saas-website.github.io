...
const guarded = function(...args) {
  ...
  if (isOwner()) {
    ...
    console.warn('Owner olarak sınırsız indirme yapıldı; kullanıcı kısıtlamaları uygulanmadı.');
    return;
  }
}
...
if (!isOwner() && localStorage.getItem('freeDownloadUsed')) {
  alert(T['free_plan_already_used']);
  return;
}

// İndirme işlemi tamamlandıysa
if (!isOwner()) localStorage.setItem('freeDownloadUsed', 'true');
